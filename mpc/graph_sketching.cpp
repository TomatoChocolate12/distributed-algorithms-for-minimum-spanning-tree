...
struct Edge {
    ...
    // Sort by weight implies we process lightest first
...
void mpc_sort(std::vector<Edge>& local_data, int rank, int size, MPI_Datatype type) {
    ...
    int off = 0;
    for(int i=0; i<size; ++i) { sc[i]=buckets[i].size(); sd[i]=off; off+=sc[i]; send_buf.insert(send_buf.end(), buckets[i].begin(), buckets[i].end()); }
...
int main(int argc, char** argv) {
    ...
    std::vector<Edge> local_edges;
    {
        ...
        int mc; MPI_Scatter(sc.data(),1,MPI_INT,&mc,1,MPI_INT,0,MPI_COMM_WORLD);
    ...
    // 2. Distributed Sort (Crucial for MST correctness)
    ...
    // Process Chunks Sequentially (Rank 0, then Rank 1...)
    // This respects the "Observation": Check edges against forests formed by *lighter* edges.
    for (int r = 0; r < size; ++r) {
        ...
        // B. Collapse Components (Boruvka-style within the stream)
        bool merged = true;
        while (merged) {
            merged = false;
    ...
    if (rank == 0) {
        ...
        std::cout << "Edges: " << mst_edges_count << std::endl;
    }
